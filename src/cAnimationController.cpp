...
void TransformMatrix(glm::mat4 & out,const aiMatrix4x4& in){// there is some type of alignment issue with my mat4 and the aimatrix4x4 class, so the copy must be manually
	/* this is directx code  https://msdn.microsoft.com/en-us/library/windows/desktop/microsoft.directx_sdk.reference.xmfloat4x4(v=vs.85).aspx
	out._11=in.a1;
	out._12=in.a2;
	out._13=in.a3;
	out._14=in.a4;
...
void cAnimEvaluator::Evaluate( float pTime, std::map<std::string, cBone*>& bones) {
	...
	for( unsigned int a = 0; a < Channels.size(); a++){
		...
		if(bonenode == bones.end()) { 
			//bugbug OUTPUT_DEBUG_MSG("did not find the bone node "<<channel->Name);
			continue;
		}
...
//bugbug					found->second->Offset.Transpose();// transpoce their matrix to get in the correct format
	...
	//bugbug OUTPUT_DEBUG_MSG("Finished loading animations with "<<Bones.size()<<" bones");
}
...
void UQTtoUDQ(glm::vec4* dual, glm::quat q, glm::vec4& tran ) {
    dual[0].x = q.x ; 
    dual[0].y = q.y ; 
    dual[0].z = q.z ; 
    dual[0].w = q.w ; 
...
cBone* SceneAnimator::CreateBoneTree( aiNode* pNode, cBone* pParent){
	...
	//bugbug internalNode->LocalTransform.Transpose();
